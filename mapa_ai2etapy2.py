...
def eksport_do_kml(df):
    if df.empty: return None
    gdf = gpd.GeoDataFrame(df, geometry=gpd.points_from_xy(df.lon, df.lat), crs="EPSG:4326")
    # Zamieniamy wszystko na tekst, bo KML bywa kapryśny przy typach mieszanych
...
# --- NOWE FUNKCJE EKSPORTU (WORD/EXCEL/CSV) ---
...
if 'source_texts' not in st.session_state: st.session_state.source_texts = {}
...
with col_info:
    ...
    # --- NOWA SEKCJA: ZAPISZ LISTĘ PUNKTÓW ---
    st.divider()
    st.write("### 💾 Zapisz listę punktów do pliku")
    c_geo, c_kml = st.columns(2)
    with c_geo:
        geo_data = eksport_do_geojson(df)
        if geo_data:
            st.download_button(
                label="📥 Pobierz listę (GeoJSON)",
                data=geo_data,
                file_name=f"moje_punkty_{datetime.now().strftime('%Y%m%d')}.geojson",
                mime="application/geo+json",
                use_container_width=True
            )
    with c_kml:
        kml_dane = eksport_do_kml(df)
        if kml_dane:
            st.download_button(
                label="📥 Pobierz listę (KML)",
                data=kml_dane,
                file_name=f"moje_punkty_{datetime.now().strftime('%Y%m%d')}.kml",
